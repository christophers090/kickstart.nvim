return {
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map('<leader>co', require('telescope.builtin').lsp_document_symbols, '[C]ode D[o]cument Symbols')
          ...
          -- Inlay hints toggle removed to avoid conflict with <leader>th terminal binding
          -- You can toggle inlay hints with :lua vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled())
        end,
      })

      -- Diagnostic Config
      -- See :help vim.diagnostic.Opts
      -- Determine if we should show diagnostic signs (flags in gutter) and virtual text
      local show_diagnostics = true
      if vim.fn.getcwd():match('slugworks') then
      ...
      vim.diagnostic.config {
        ...
        underline = show_diagnostics and { severity = vim.diagnostic.severity.ERROR } or false,
        signs = show_diagnostics and (vim.g.have_nerd_font and {
          text = {
  ...
  { -- Autocompletion
    ...
    opts = {
      ...
      -- Disable cmdline completion to avoid suggestions in noice popup
